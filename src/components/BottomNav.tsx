    ...
    <>
      ...
      {showMenu && !isCoach && (
        <div style={{
          ...
          <button
            onClick={() => handleMenuOption('/organize')}
            style={{
              display: 'flex',
              alignItems: 'center',
              gap: '14px',
          ...
          >
            <span style={{
              ...
              background: '#E8F4FC', borderRadius: '12px',
              display: 'flex', alignItems: 'center', justifyContent: 'center',
              fontSize: '22px'
            }}>🎾</span>
            <div style={{ textAlign: 'left' }}>
              <p style={{ fontWeight: 700, fontSize: '16px', color: '#111' }}>Organizza partita</p>
              <p style={{ fontSize: '12px', color: '#999' }}>Pianifica e invita amici</p>
            </div>
          </button>

          <button
            onClick={() => handleMenuOption('/quick-match')}
            style={{
              display: 'flex',
              alignItems: 'center',
              gap: '14px',
          ...
          >
            ...
            <div style={{ textAlign: 'left' }}>
              <p style={{ fontWeight: 700, fontSize: '16px', color: '#111' }}>Registra risultato</p>
              <p style={{ fontSize: '12px', color: '#999' }}>Salva una partita giocata</p>
            </div>
          </button>
      ...
      <nav className="fixed bottom-0 left-0 right-0 bg-white z-50" style={{
        ...
        <div className="max-w-lg mx-auto flex justify-around items-center h-16 relative">
          ...
          {/* Home */}
          <Link href="/dashboard" className="flex flex-col items-center justify-center flex-1 h-full" style={{ color: isActive('/dashboard') ? accent : '#999' }}>
          ...
          {/* Tab 2 */}
          <Link href={isCoach ? '/connections' : '/leagues'} className="flex flex-col items-center justify-center flex-1 h-full" style={{ color: isActive(isCoach ? '/connections' : '/leagues') ? accent : '#999' }}>
            {isCoach ? (
          ...
          <div className="flex flex-col items-center justify-center flex-1 h-full">
            <button
              ...
              style={{
                background: showMenu ? '#666' : accent,
                boxShadow: `0 6px 20px ${showMenu ? 'rgba(0,0,0,0.2)' : accentShadow}`,
          ...
          {/* Tab 4 */}
          <Link href={isCoach ? '/videos' : '/calendar'} className="flex flex-col items-center justify-center flex-1 h-full" style={{ color: isActive(isCoach ? '/videos' : '/calendar') ? accent : '#999' }}>
            {isCoach ? (
          ...
          {/* Profilo */}
          <Link href="/profile" className="flex flex-col items-center justify-center flex-1 h-full" style={{ color: isActive('/profile') ? accent : '#999' }}>
